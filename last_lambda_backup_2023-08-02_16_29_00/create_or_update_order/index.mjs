...
export const handler = async (event) => {
  const { seller_email, buyer_phone_number, recipe_name, recipe_price, recipe_quantity } = event;
  ...
  for(let i = 0; i < recipe_name.length; i++) {
    const params = {
      FunctionName: 'search_lambda_with_key_and_partition',
       Payload: JSON.stringify({
          ...
          partition_key_value: seller_email
        })
    };
    triples.push({
      M:{
    ...
    try {
       ...
       const responsePayload = JSON.parse(textDecoder.decode(response.Payload));
        ...
        if (!responsePayload) {
         throw new Error(`No item found for recipe_name=${recipe_name[i]}, user_identifier=${seller_email}`);
        }
     ...
     return;
      ...
      try {
        const putItemCommand = new PutItemCommand({
            TableName: "orders",
            Item: {
                seller_email_adress: { S: seller_email },
