...
export const handler = async (event) => {
  ...
  let existingOrder;
  try {
    existingOrder  = await dynamoDbClient.send(new GetItemCommand(orderParams));
    if (!existingOrder.Item) {
      console.log(`No order found for seller '${seller_email_adress}' and buyer '${buyer_phone_number}'`);
      return;
    }
   } 
    catch (error) {
  ...
  let triples = existingOrder?.Item?.recipes?.L || [];
  ...
  for(let i = 0; i < recipe_name.length; i++) {
    ...
    // Check if the recipe already exists in triples
    const existingTripleIndex = triples.findIndex(triple => triple.M.recipe_name.S === recipe_name[i]);

    if (existingTripleIndex !== -1) {
      // Update the existing triple's quantity
      triples[existingTripleIndex].M.recipe_quantity.S = String(Number(triples[existingTripleIndex].M.recipe_quantity.S) + Number(recipe_quantity[i]));
    } else {
      // Push a new triple
      triples.push({
        M:{
          ...
          recipe_quantity:{S:recipe_quantity[i]},
        },
      });
    }
  ...
  try {
    ...
    console.log("trying to update the order")
