...
export default function Recipes({className, header, description}: IRecipeProps) {
    ...
    const createRecipeFromData = (recipeData: any, recipeId: number) => {
        ...
        const ingredients = recipeData.ingredients.L.map((ingredientData: any) => {
            ...
            const ingredientPrice=parseFloat(ingredientData.M.ingredient_price.N);
            const ingredientQuantity = parseFloat(ingredientData.M.quantity.N);
            return { id: ingredientId, name: ingredientName, minCost: ingredientPrice,avgCost:ingredientPrice,maxCost:ingredientPrice, quantity: ingredientQuantity };
        });

        const totalCost = ingredients.reduce((total: number, ingredient: IngredientType) => {
            if (ingredient) {
                return total + (ingredient.minCost * ingredient.quantity);
            }
            return total;
        }, 0);

        return { id: `#${recipeId}`, name: recipeData.recipe_name.S,price:recipeData.recipe_price.N, ingredients, totalCost };
    };
